...
4. Outputs a `new_repos_<random>.yaml` file in
   `~/.config/cloninator/config.yaml.d/` with nested directory structure
5. With `--split-groups`, writes one file per group using the group name as the
   filename prefix (for example, `all_new_repos_<random>.yaml`)

!!! note
Generated files are loaded automatically as split configuration. Review the
generated content and rename or edit it as needed.
